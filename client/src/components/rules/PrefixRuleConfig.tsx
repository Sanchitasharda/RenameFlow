import { useState } from 'react';
import { PrefixRule } from '../../types/Rule';
import { useRuleStore } from '../../store/useRuleStore';

interface PrefixRuleConfigProps {
  rule: PrefixRule;
}

/**
 * Configuration panel for prefix rules
 */
export function PrefixRuleConfig({ rule }: PrefixRuleConfigProps) {
  const updateRule = useRuleStore((state) => state.updateRule);
  const [prefix, setPrefix] = useState(rule.config.text);

  const handleSave = () => {
    updateRule(rule.id, {
      config: { text: prefix },
    });
  };

  return (
    <div className="space-y-4">
      <div>
        <label
          htmlFor={`prefix-${rule.id}`}
          className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2"
        >
          Prefix Text
        </label>
        <input
          id={`prefix-${rule.id}`}
          type="text"
          value={prefix}
          onChange={(e) => setPrefix(e.target.value)}
          onBlur={handleSave}
          placeholder="e.g., final_, 2025_"
          className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500"
        />
      </div>

      <div className="bg-gray-50 dark:bg-gray-900 p-3 rounded-lg">
        <p className="text-sm text-gray-600 dark:text-gray-400 mb-1">
          Preview:
        </p>
        <p className="font-mono text-gray-900 dark:text-white">
          {prefix || '(empty)'}
          <span className="text-gray-500">filename.jpg</span>
        </p>
      </div>
    </div>
  );
}
